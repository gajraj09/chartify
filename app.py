...
entryprice = None            # price at which we are currently "entry" (open position)
open_side = None             # "buy" or "sell" for the open position
running_pnl = 0.0
...
# Exit fill tracking
exit_fill_pending = 0      # flag indicating an exit order is pending/being tracked (0/1)
exit_fill_count = 0        # number of exit fills observed
exit_fill_total = 0        # number of exit requests made / exit orders created
exit_target_price = None   # the price we expect the market to hit to consider exit filled
...
def save_state():
    ...
    global exit_fill_pending, exit_fill_count, exit_fill_total, exit_target_price, open_side
    ...
    try:
        state = {
            ...
            # exit-related state
            "exit_fill_pending": int(exit_fill_pending),
            "exit_fill_count": int(exit_fill_count),
            "exit_fill_total": int(exit_fill_total),
            "exit_target_price": float(exit_target_price) if is_valid_price(exit_target_price) else None,
        }
...
def load_state():
    ...
    try:
        ...
        # exit related
        exit_fill_pending = doc.get("exit_fill_pending", 0)
        exit_fill_count = doc.get("exit_fill_count", 0)
        exit_fill_total = doc.get("exit_fill_total", 0)
        exit_target_price = doc.get("exit_target_price", None)
...
def send_webhook(trigger_time_iso: str, entry_price_in: float, side: str, status_fun: str):
    """
    Unified send_webhook: sends payload then updates internal bookkeeping.
    status_fun accepts "entry" / "enter" (entry) or "exit".
    side should be "buy" or "sell" describing the webhook intent.
    """
    ...
    # Normalize status
    s = status_fun.lower() if isinstance(status_fun, str) else status_fun

    # Build payload and POST (best-effort)
    payload = {
        "symbol": SYMBOL.upper(),
        "side": side,
        "quantity": quantity,
        "price": entry_price_in,
        "status": "entry" if s in ("enter", "entry") else s,
        "secret": secret,
    }
    try:
    ...
    # Now update internal bookkeeping depending on intent
    status = "entry" if s in ("enter", "entry") else s

    if status == "exit":
        # Initiate exit tracking: DO NOT immediately drop entryprice until exit is confirmed.
        if entryprice is None:
            print("[WEBHOOK] Exit requested but no stored entryprice; ignoring exit tracking.")
        else:
            # track the exit target price so update_fillcheck can detect the fill
            exit_target_price = float(entry_price_in)
            exit_fill_pending = 1
            exit_fill_total += 1
            # store which side we are closing (open_side)
            # do not compute PnL here — compute only when exit is filled in update_fillcheck
            print(f"[WEBHOOK] Exit initiated towards {fmt_price(exit_target_price)} for open_side={open_side}")

    elif status == "entry":
        # Entry requested: if an entry is already open, account for closing it (implicit close)
        if entryprice is not None and open_side is not None:
            # implicitly close previous (for accounting) at this new price
            pnl = calculate_pnl(entryprice, float(entry_price_in), open_side)
            if fillcheck == 0:
                initial_balance += pnl
            running_pnl += pnl
            lastpnl = pnl
            print(f"[ENTRY] Closing previous open for accounting. pnl={fmt_price(pnl)}")
        else:
            print(f"[ENTRY] Opening new position at {fmt_price(entry_price_in)}")

        # Open new position
        entryprice = float(entry_price_in)
        open_side = side
        totaltradecount += 1
        fillcheck = 1  # we expect this entry to fill
        # reset any exit tracking because we are opening new position
        exit_fill_pending = 0
        exit_target_price = None

    # update last side bookkeeping
...
def update_fillcheck(trade_price: float):
    """
    Handles:
     - entry fills (when a trade reaches the entryprice)
     - exit fills (when market reaches the exit_target_price)
    When an exit fills, compute PnL, update balances, clear entry/open_side.
    """
    global fillcheck, fillcount, entryprice, LastSide, status
    global totaltradecount, unfilledpnl, exit_fill_pending, exit_fill_count, lastpnl, initial_balance, running_pnl, open_side

    if not is_valid_price(trade_price):
        return

    was_fill = False

    # Entry fill detection
    if entryprice is not None and status == "entry" and fillcheck == 1:
        if open_side == "buy" and trade_price <= entryprice:
            fillcheck = 0
            fillcount += 1
            was_fill = True
            print(f"[FILL] Entry filled (buy) at {fmt_price(trade_price)}")
        elif open_side == "sell" and trade_price >= entryprice:
            fillcheck = 0
            fillcount += 1
            was_fill = True
            print(f"[FILL] Entry filled (sell) at {fmt_price(trade_price)}")

    # Exit fill detection (based on exit_target_price)
    if exit_fill_pending == 1 and exit_target_price is not None:
        if open_side == "buy" and trade_price >= exit_target_price:
            # exited a long position
            exit_fill_pending = 0
            exit_fill_count += 1
            was_fill = True
            # compute pnl and update accounting
            pnl = calculate_pnl(entryprice, exit_target_price, open_side)
            if fillcheck == 0:
                initial_balance += pnl
            running_pnl += pnl
            lastpnl = pnl
            ...
            entryprice = None
            open_side = None
            print(f"[EXIT-FILL] Exit filled (long) at {fmt_price(exit_target_price)} pnl={fmt_price(pnl)}")
        elif open_side == "sell" and trade_price <= exit_target_price:
            # exited a short position
            exit_fill_pending = 0
            exit_fill_count += 1
            was_fill = True
            pnl = calculate_pnl(entryprice, exit_target_price, open_side)
            ...
            entryprice = None
            open_side = None
            print(f"[EXIT-FILL] Exit filled (short) at {fmt_price(exit_target_price)} pnl={fmt_price(pnl)}")
        ...
        if stream and "kline" in stream:
            ...
            if new_candle_detected:
                ...
                if status == "exit":
                    ...
                    try:
                        # For an "entry"->"exit" flow you previously used 'open_val' as the exit price.
                        # If you desire an automatic exit on new candle, send an exit webhook accordingly.
...
def update_display(_):
    ...
    stats_html = [
        ...
        html.Div(f"ExitFillPending: {exit_fill_pending}"),
        html.Div(f"ExitFillCount: {exit_fill_count}"),
